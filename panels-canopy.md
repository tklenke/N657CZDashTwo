# Canopy

::: pending
**PENDING** — 2026-04-17
:::

## Description

The canopy is the primary transparent enclosure on N657CZ. It is a blown acrylic panel, vacuum-formed to the compound curve of the cockpit opening. The canopy provides weather protection, aerodynamic fairing, and occupant visibility forward and to the sides. It is located above and aft of the instrument panel and hinges on the right side.

The canopy is raised and lowered by an electric actuator. The actuator holds the canopy in any position — fully open, fully closed, or any intermediate position. It is controlled by a three-position switch, (OPEN)–OFF–(CLOSE), located at the Instrument Panel (IP). The switch is accessible from outside the aircraft through the canopy access door on the left side of the fuselage, just aft of the instrument panel. 

A manual latch provides a secondary means of securing the canopy closed. The latch bar runs parallel to the canopy rail. To release: reach through the canopy access door, grasp the latch bar, and slide it aft.

## Specifications

| Parameter | Value |
|-----------|-------|
| Material | Blown acrylic plastic |
| Approximate dimensions | 30 in. wide × 48 in. long |
| Hinge | Right side |
| Opening mechanism | Electric actuator; three-position switch (OPEN)–OFF–(CLOSE) |
| Secondary latch | Manual latch bar, parallel to canopy rail |
| Actuator switch access | Aircraft access door, left side of fuselage, just aft of IP (approx. 6 in. × 6 in., keyed lock) |

> **NOTE:** Acrylic plastic meets MIL-P-5425 (regular acrylic) or MIL-P-8184 (craze-resistant acrylic). Use equivalent acrylic material for any replacement panel. (AC 43.13, ch03_p018, ch03_p026)

## Inspection

Inspect the canopy at each condition inspection.

**Acrylic surface:**

- **Crazing**: Look for a network of fine surface cracks. Crazing results from stress, solvent contact, or heat. Any crazing warrants evaluation for replacement. (AC 43.13, ch03_p020)
- **Cracks**: At the first sign of crack development, stop-drill the crack ends with a #30 (1/8 in.) drill to prevent further propagation. Schedule for repair or replacement. (AC 43.13, ch03_p024, ch03_p040)
- **Scratches**: Deep scratches that impair pilot visibility require repair or replacement. Minor surface scratches may be polished out per the procedure in AC 43.13, ch03_p040.
- **Optical clarity**: Check for hazing, yellowing, or optical distortion.

> **NOTE:** Replace rather than repair extensively damaged transparent plastic whenever possible. Repairs must not be located in the pilot's line of vision. (AC 43.13, ch03_p024)

**Seal and structure:**

- **Canopy seal**: @@TOM: Describe seal type. Inspect for compression set, tears, gaps, or missing sections that would allow water intrusion or air leaks.
- **Electric actuator**: Verify the canopy opens and closes smoothly on command and holds position without drifting. Check for unusual noise or hesitation during operation.
- **Manual latch bar**: Check for free, smooth movement along the rail. Verify positive engagement and secure hold when latched closed. Inspect for corrosion, wear, or binding.

## Procedures

- **Cleaning**: See [[Canopy Cleaning|panels-canopy-cleaning]]
